...
process REGISTRATION_COBRALABANTS {
    ...
    if ( task.ext.convergence != '1e-6' ) args += " --convergence $task.ext.convergence"
    if ( task.ext.skip_linear ) args += " --skip-linear"
    if ( task.ext.linear_metric != 'Mattes' ) args += " --linear-metric $task.ext.linear_metric"
    ...
    if ( task.ext.final_iterations_linear != '50' ) args += " --final-iterations-linear $task.ext.final_iterations_linear"
    if ( task.ext.kmeans_transformed_linear ) args += " --kmeans-transformed-linear"
    if ( task.ext.skip_nonlinear ) args += " --skip-nonlinear"
    if ( task.ext.syn_control != '0.4,4,0' ) args += " --syn-control $task.ext.syn_control"
    if ( task.ext.syn_metric != 'CC[4]' ) args += " --syn-metric $task.ext.syn_metric"
    ...
    if ( task.ext.final_iterations_nonlinear != '20' ) args += " --final-iterations-nonlinear $task.ext.final_iterations_nonlinear"
